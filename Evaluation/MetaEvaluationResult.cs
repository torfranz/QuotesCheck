    ...
    internal class MetaEvaluationResult
    {
        ...
        public void Save(string folder, long duration)
        {
            ...
            Json.Save(
                Path.Combine(
                    folder,
                    $"Evaluation-MetaResult-[{this.Iteration}]-[{this.Performance.TotalGain:F0}% +{this.Performance.PositiveTrades} -{this.Performance.NegativeTrades}]-{duration}ms-{now:yyyy-MM-dd-HH-mm-ss}.json"),
                this);
        }
    }
}